...
type Application = {
  ...
  strategy: string;
  baseFee: number;   // NOVA
  bonusFee: number;  // NOVA
  minStake: number;  // NOVA to stake (tipster side)
...
export default function ApplyAsTipster() {
  ...
  // simple bus: intercept window alert from ConnectWalletButton to sniff the fake address (demo only)
  // In real integration we’ll read address from wagmi.
  useEffect(() => {
    // our dummy button passes only boolean, so we can't read address directly.
    // We'll just display "Connected" state; address will be shown by the button itself.
  }, []);
  ...
  function simulateStake() {
    ...
    alert(`✅ Demo: Staked ${app.minStake} NOVA (simulated).`);
  }

  const canSubmit = useMemo(() => {
    return connected && app.name && app.bio && app.sports && app.strategy && app.contacts && app.agree;
  }, [connected, app]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <ConnectWalletButton onChange={(ok) => { setConnected(ok); setAddress(ok ? '0xA1b2...9F3c' : null); }} />
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          <Card className="bg-slate-900/60 border-slate-800">
            ...
            <CardContent className="text-sm text-slate-300 space-y-2">
              <p><strong>Fees split:</strong> Tip price = <em>Base fee</em> + <em>Outcome bonus</em>.</p>
              ...
              <p className="text-xs text-slate-400">All payments are in NOVA via escrow contract. Results are verified by oracle before settlement.</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-slate-900/60 border-slate-800">
            ...
            <CardContent className="text-sm text-slate-300 space-y-2">
              <p>You must stake NOVA to list. Poor performance over time can lower rank or increase required stake.</p>
              <Button onClick={simulateStake} className="rounded-xl w-full">Simulate Stake {app.minStake} NOVA</Button>
              <p className="text-xs text-slate-400">Demo only. On mainnet/testnet we’ll guide you through real staking with wallet.</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-6 text-sm text-slate-300">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                    value={app.name}
                ...
                <div>
                  <label className="block text-slate-400 mb-1">Primary sports/leagues *</label>
                  <input
                    className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                    value={app.sports}
              ...
              <div>
                ...
                <textarea
                  className="w-full min-h-[90px] rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                  value={app.bio}
              ...
              <div>
                <label className="block text-slate-400 mb-1">Strategy overview *</label>
                <textarea
                  className="w-full min-h-[120px] rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                  value={app.strategy}
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-slate-400 mb-1">Base fee (NOVA) *</label>
                  <input
                    type="number"
                    className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                    value={app.baseFee}
                    ...
                    min={0}
                  />
                  <p className="text-[11px] text-slate-500 mt-1">Tipster keeps this even if the pick loses.</p>
                </div>
                <div>
                  <label className="block text-slate-400 mb-1">Outcome bonus (NOVA) *</label>
                  <input
                    type="number"
                    className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                    value={app.bonusFee}
                    ...
                    min={0}
                  />
                  <p className="text-[11px] text-slate-500 mt-1">Paid only if the pick wins (buyer credit otherwise).</p>
                </div>
                <div>
                  <label className="block text-slate-400 mb-1">Stake amount (NOVA) *</label>
                  <input
                    type="number"
                    className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                    value={app.minStake}
                    ...
                    min={100}
                  />
                  <p className="text-[11px] text-slate-500 mt-1">Required to list. Helps align incentives.</p>
                </div>
              </div>

              <div>
                <label className="block text-slate-400 mb-1">Contact (Telegram / Email) *</label>
                <input
                  className="w-full rounded-xl bg-slate-950/60 border border-slate-800 px-3 py-2 outline-none"
                  value={app.contacts}
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-sm text-slate-300">
                ...
                <p className="text-slate-400">After submit, your profile will appear in the leaderboard once approved.</p>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button variant="secondary" className="rounded-xl" onClick={() => setApp(p=>({ ...p, status: 'draft' }))}>
                  Save as draft
                </Button>
              </div>
            </CardContent>
