...
import {
  Wallet, Copy, Check, TrendingUp, Trash2, ArrowLeft, Calculator,
  User, Settings, ShieldCheck, LogIn, UserPlus, X, CalendarClock,
  Trophy, Mail, Facebook
} from 'lucide-react';
...
/* ====== DEMO DEPOSIT ADDRS ====== */
const DEPOSIT_ADDRESSES: Record<string, { address: string; network: string; note?: string }> = {
  USDT: { address: '0xUSDT-DEMO-ADDR-1234567890ABCDEF', network: 'BEP-20 (BSC)', note: 'Send only USDT on BNB Smart Chain.' },
  BTC:  { address: 'bc1q-demo-btc-address-2k4u7a3yx',   network: 'Bitcoin (BTC)', note: 'Native BTC only.' },
  ETH:  { address: '0xETH-DEMO-ADDR-ABCDEF1234567890', network: 'ERC-20 (Ethereum)', note: 'Send only ERC-20 ETH.' },
  NOVA: { address: '0xNOVA-DEMO-ADDR-00112233445566',  network: 'BEP-20 (BSC)', note: 'Project token on BSC.' },
};

/* ====== TYPES ====== */
type Sport = 'Football' | 'Tennis' | 'Basketball' | 'Cricket';
type Match = {
  id: string; sport: Sport; league: string; startTs: number;
  teamA: string; teamB: string; market: '1x2' | 'moneyline';
  odds: { A: number; Draw?: number; B: number }; trending?: boolean;
};
...
function useAuth() {
  ...
  useEffect(() => {
    try { const raw = localStorage.getItem('authUser'); if (raw) setUser(JSON.parse(raw)); } catch {}
  }, []);

  function getDb(): Record<string, UserRec> {
    try { return JSON.parse(localStorage.getItem('usersDb') || '{}'); } catch { return {}; }
  }
  function putDb(db: Record<string, UserRec>) { localStorage.setItem('usersDb', JSON.stringify(db)); }

  function signUp({ email, username, password }: { email: string; username: string; password: string }) {
    const db = getDb();
    if (db[email]) throw new Error('Email already exists.');
    const rec: UserRec = { id: crypto.randomUUID(), email, username, password };
    db[email] = rec; putDb(db);
    localStorage.setItem('authUser', JSON.stringify({ id: rec.id, email: rec.email, username: rec.username }));
  ...
  function signUpWithGoogleDemo() {
    ...
    const db = getDb();
    if (!db[email]) { db[email] = { id: crypto.randomUUID(), email, username, password }; putDb(db); }
    localStorage.setItem('authUser', JSON.stringify({ id: db[email].id, email, username }));
    setUser({ id: db[email].id, email, username });
  }

  function signOut() { localStorage.removeItem('authUser'); setUser(null); }
...
export default function BetPage() {
  ...
  const [baseNow, setBaseNow] = useState<number>(() => (typeof window !== 'undefined' ? Date.now() : 0));
  const isReady = baseNow !== 0;
  useEffect(() => { setBaseNow(Date.now()); }, []);

  const MATCHES: Match[] = useMemo(() => {
    const t = isReady ? baseNow : 0;
    return [
      { id: 'm1', sport: 'Football',   league: 'EPL',              startTs: t + 60*60*1000,   teamA: 'Arsenal',      teamB: 'Newcastle',  market: '1x2',      odds: { A: 1.86, Draw: 3.60, B: 4.10 }, trending: true },
      { id: 'm2', sport: 'Football',   league: 'La Liga',          startTs: t + 2*60*60*1000, teamA: 'Real Madrid',  teamB: 'Sevilla',    market: '1x2',      odds: { A: 1.92, Draw: 3.90, B: 4.80 } },
      { id: 'm3', sport: 'Tennis',     league: 'ATP 500',          startTs: t + 90*60*1000,   teamA: 'Ruud',         teamB: 'De Minaur',  market: 'moneyline',odds: { A: 1.72,                   B: 2.10 }, trending: true },
      { id: 'm4', sport: 'Tennis',     league: 'WTA 250',          startTs: t + 3*60*60*1000, teamA: 'Gauff',        teamB: 'Kalinina',   market: 'moneyline',odds: { A: 1.48,                   B: 2.55 } },
      { id: 'm5', sport: 'Basketball', league: 'EuroLeague',       startTs: t + 2.5*60*60*1000,teamA: 'Madrid',      teamB: 'Fenerbahçe', market: 'moneyline',odds: { A: 1.70,                   B: 2.15 }, trending: true },
      { id: 'm6', sport: 'Basketball', league: 'NBA (Preseason)',  startTs: t + 4*60*60*1000, teamA: 'Warriors',     teamB: 'Lakers',     market: 'moneyline',odds: { A: 1.90,                   B: 1.95 } },
      ...
      { id: 'm8', sport: 'Cricket',    league: 'ODI',              startTs: t + 7*60*60*1000, teamA: 'England',      teamB: 'Pakistan',   market: 'moneyline',odds: { A: 1.80,                   B: 2.00 } },
    ];
  }, [baseNow, isReady]);

  const userKey = user?.email ?? 'guest';
  const [balances, setBalances] = useState<Record<string, number>>({ USDT: 0, BTC: 0, ETH: 0, NOVA: 0 });
  useEffect(() => {
    try {
  ...
  const saveBalances = (next: Record<string, number>) => {
    setBalances(next);
    try { localStorage.setItem(`balances:${userKey}`, JSON.stringify(next)); } catch {}
  };
  ...
  const demoCredit = (amount: number) => {
    if (!user) { alert('Please sign up / log in first.'); return; }
    ...
    <div className={pageBg}>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-slate-300 mr-2">Balances:</span>
          {(['USDT','BTC','ETH','NOVA'] as const).map(c => (
            <Badge key={c} variant="outline" className="font-mono text-slate-100 border-slate-600">
              {c}: {balances[c] ?? 0}
            </Badge>
          ))}
        </div>
          ...
          <Card className={panel}>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <span className={chip}>Secure credits</span>
              </div>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-slate-200">
              <div>
                <label className="block text-slate-200 mb-1">Select currency</label>
                <select className={`w-full ${field}`} value={currency} onChange={(e)=>setCurrency(e.target.value as any)} disabled={!user}>
              ...
              <div className="rounded-xl bg-slate-900/85 border border-slate-700 p-3 text-slate-200">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-slate-100 mt-1">Network: <Badge variant="secondary">{dep.network}</Badge></div>
                  </div>
                  <CopyButton text={dep.address} />
                </div>
                {dep.note && <p className="text-[11px] text-slate-300 mt-2">{dep.note} Wrong-chain transfers will be lost.</p>}
              </div>
              ...
              <div className="rounded-xl bg-slate-900/85 border border-slate-700 p-3">
                ...
                <DemoCredit currency={currency} onCredit={demoCredit} disabled={!user} />
                {!user && <p className="text-[11px] text-amber-300 mt-2">Sign up / log in to enable demo credit.</p>}
                <p className="text-[11px] text-slate-400 mt-2">In production: backend detects deposits and credits after confirmations.</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className={panel}>
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <span className={chip}><Trophy className="h-3.5 w-3.5 inline mr-1" /> Hot</span>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              {trending.map(m => (
                <div key={m.id} className="rounded-lg bg-slate-900/85 border border-slate-700 p-3 text-sm text-slate-200">
                  <div className="flex items-center justify-between">
                    <div className="font-semibold text-slate-100 truncate">{m.teamA} vs {m.teamB}</div>
                    <Badge variant="secondary"><TimeBadge ts={m.startTs} enabled={isReady} /></Badge>
                  </div>
                  <div className="text-slate-300">{m.league} • {m.sport}</div>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <div className="flex flex-wrap gap-2">
            {(['Football','Tennis','Basketball','Cricket'] as Sport[]).map(s => (
              <Button
                key={s}
                variant="secondary"
                className={`rounded-full px-4 ${s===sport ? 'bg-sky-600 text-white hover:bg-sky-600' : ''} flex items-center gap-2`}
                onClick={()=>setSport(s)}
              >
                <span aria-hidden>{SPORT_ICONS[s]}</span>{s}
              </Button>
            ))}
          </div>
              ...
              <Card key={m.id} className={panel}>
                <CardHeader>
                  <CardTitle className="text-base flex items-center justify-between text-slate-100">
                    <span className="flex items-center gap-2"><CalendarClock className="h-4 w-4" /> {m.league}</span>
                    <Badge variant="secondary"><TimeBadge ts={m.startTs} enabled={isReady} /></Badge>
                  </CardTitle>
                </CardHeader>
                  ...
                  <>
                    <div className="flex items-center gap-4">
                      <label className="flex items-center gap-2 text-slate-200">
                        <input type="radio" checked={mode==='parlay'} onChange={()=>setMode('parlay')} /> Parlay
                      </label>
                      <label className="flex items-center gap-2 text-slate-200">
                        <input type="radio" checked={mode==='single'} onChange={()=>setMode('single')} /> Singles (equal split)
                      </label>
                    </div>

                    <div className="space-y-2">
                      {slip.map(p => (
                        <div key={p.id} className="rounded-lg bg-slate-900/85 border border-slate-700 p-3 flex items-center gap-3">
                          <Badge>{p.sport}</Badge>
                          <div className="flex-1 min-w-0">
                            <div className="text-slate-100 truncate">{p.label}</div>
                            <div className="text-slate-300 text-xs">Odds @{p.odds.toFixed(2)}</div>
                          </div>
                          <Button size="sm" variant="secondary" className="rounded-lg" onClick={()=>removePick(p.id)}>
                    ...
                    <div className="grid md:grid-cols-3 gap-3 items-end">
                      <div className="md:col-span-2">
                        <label className="block text-slate-200 mb-1">
                          {mode==='parlay' ? `Stake (${betCurrency})` : `Total stake (${betCurrency})`}
                        </label>
                        <input type="number" min={1} className={`w-full ${field}`} value={stake} onChange={e=>setStake(Number(e.target.value))}/>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {[10,25,50].map(v=>(
                            <Button key={v} size="sm" variant="secondary" className="rounded-lg" onClick={()=>setStake(v)}>+{v}</Button>
                          ))}
                          <Button size="sm" variant="secondary" className="rounded-lg" onClick={()=>setStake(Math.floor(balances[betCurrency]||0))}>MAX</Button>
                          <select className="ml-auto bg-slate-900/85 text-slate-100 border border-slate-700 rounded-lg px-2 py-1 text-xs" value={betCurrency} onChange={e=>setBetCurrency(e.target.value as any)}>
                      ...
                      <div className="rounded-xl bg-slate-900/85 border border-slate-700 p-3">
                        ...
                        <div className="text-slate-100 font-semibold text-lg">{payout.potential} {betCurrency}</div>
                        <div className="text-slate-300 text-[11px]">{payout.details}</div>
                      </div>
                    </div>

                    <div className="flex items-center justify-between">
                      <Button variant="secondary" className="rounded-xl" onClick={clearSlip}>Clear</Button>
                      <Button className="rounded-xl" onClick={placeBet} disabled={!user}>Place bet (demo)</Button>
                    </div>

                    {!user && <div className="text-[11px] text-amber-300">Sign up or log in to place bets.</div>}
                  </>
                )}
              </CardContent>
            </Card>
      ...
      {showLogin && (
        <AuthModal title="Log in" onClose={()=>setShowLogin(false)}>
          <LoginForm
            onSubmit={(email, pass) => {
              try { signIn(email, pass); setShowLogin(false); }
      ...
      {showSignup && (
        <AuthModal title="Create account" onClose={()=>setShowSignup(false)}>
          <SignupForm
            onSubmit={(payload) => {
              try {
                if (!payload.agree) { alert('Please agree to Terms & Privacy'); return; }
                signUp({ email: payload.email, username: payload.username, password: payload.password });
                setShowSignup(false);
              } catch (e:any) { alert(e.message || 'Sign up failed'); }
            }}
            onGoogle={() => { signUpWithGoogleDemo(); setShowSignup(false); }}
            onSignInLink={() => { setShowSignup(false); setShowLogin(true); }}
          />
        </AuthModal>
      )}
    </div>
...
/* ====== SMALL COMPONENTS ====== */
function OddsBtn({ label, odds, onClick }: { label: string; odds: number; onClick: () => void }) {
  return (
    <Button
...
function AuthModal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return (
...
function LoginForm({ onSubmit, onForgot }: { onSubmit: (email: string, pass: string) => void; onForgot: () => void }) {
  const [email, setEmail] = useState(''); const [pass, setPass] = useState('');
  return (
    <form className="space-y-4" onSubmit={(e)=>{e.preventDefault(); onSubmit(email, pass);}}>
      <div>
      ...
      <div>
        ...
        <button type="button" className="mt-2 text-sky-300 text-sm hover:underline" onClick={onForgot}>
          Forgot password?
        </button>
      </div>
      <div className="flex items-center justify-end gap-2 pt-2">
        <Button type="submit" className="rounded-xl"><LogIn className="h-4 w-4 mr-2" /> Log in</Button>
      </div>
    </form>
  );
}

/* Social logos (simple SVG) */
function GoogleLogo() {
  return (
...
function LineLogo() {
  return (
    <svg width="18" height="18" viewBox="0 0 100 100" aria-hidden>
      <rect x="10" y="10" width="80" height="80" rx="20" fill="#06C755" />
      <rect x="28" y="28" width="44" height="30" rx="6" fill="white" />
    </svg>
  );
}
function TwitchLogo() {
  return (
    <svg width="18" height="18" viewBox="0 0 240 240" aria-hidden>
      <path fill="#9146FF" d="M30 0h210v150l-60 60h-60l-30 30H60v-30H0V30z"/>
      <path fill="#FFF" d="M75 55h30v60H75zM135 55h30v60h-30z"/>
    </svg>
  );
}

function SocialRow({ onGoogle }: { onGoogle: () => void }) {
  const base = 'h-10 w-10 grid place-items-center rounded-md bg-slate-800 hover:bg-slate-700 border border-slate-600';
  return (
    <div className="flex items-center justify-center gap-3 pt-2">
      <button className={base} title="Facebook (demo)" onClick={onGoogle}><Facebook className="h-4 w-4" /></button>
      <button className={base} title="Google (demo)" onClick={onGoogle}><GoogleLogo /></button>
      <button className={base} title="LINE (demo)" onClick={onGoogle}><LineLogo /></button>
      <button className={base} title="Twitch (demo)" onClick={onGoogle}><TwitchLogo /></button>
    </div>
  );
}

function SignupForm({
  onSubmit, onGoogle, onSignInLink,
}: {
  onSubmit: (p: { username: string; email: string; password: string; agree: boolean }) => void;
  onGoogle: () => void;
  onSignInLink: () => void;
}) {
  const [username, setUsername] = useState(''); const [email, setEmail] = useState(''); const [pass, setPass] = useState(''); const [agree, setAgree] = useState(false);
  return (
    <form className="space-y-4" onSubmit={(e)=>{e.preventDefault(); onSubmit({ username, email, password: pass, agree });}}>
      <div>
        <label className="block text-slate-200 text-sm mb-1">Username</label>
        <input className={field} placeholder="3–14 characters" value={username} onChange={e=>setUsername(e.target.value)} />
      </div>
      <div>
        <label className="block text-slate-200 text-sm mb-1">Email</label>
        <input className={field} type="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} />
      ...
      <Button type="submit" className="rounded-xl w-full"><UserPlus className="h-4 w-4 mr-2" /> Continue</Button>
      ...
      <div className="pt-3 text-center text-sm text-slate-300">
        ...
        <button type="button" className="text-sky-300 hover:underline" onClick={onSignInLink}>Sign in</button>
      </div>
    </form>
  );
}
...
function DemoCredit({ currency, onCredit, disabled }: { currency: 'USDT'|'BTC'|'ETH'|'NOVA', onCredit: (amt:number)=>void, disabled?: boolean }) {
  const [amount, setAmount] = useState<number>(50);
  const [tx, setTx] = useState('');
  return (
    <div className="grid gap-2">
      <input placeholder={`Amount (${currency})`} type="number" min={1} className={field} value={amount} onChange={e=>setAmount(Number(e.target.value))} disabled={disabled}/>
      <input placeholder="Tx hash / reference (demo)" className={field} value={tx} onChange={e=>setTx(e.target.value)} disabled={disabled}/>
      <Button className="rounded-lg" onClick={()=>onCredit(amount)} disabled={disabled}>I sent deposit (credit demo)</Button>
    </div>
  );
}
